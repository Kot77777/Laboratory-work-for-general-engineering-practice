...
with open("values_ADC_for_20mm_kalib.txt", "r") as data:
    values_ADC_for_20mm_kalib = np.array([i for i in data.read().split("\n")])

with open("values_ADC_for_40mm_kalib.txt", "r") as data:
    values_ADC_for_40mm_kalib = np.array([i for i in data.read().split("\n")])

with open("values_ADC_for_60mm_kalib.txt", "r") as data:
    values_ADC_for_60mm_kalib = np.array([i for i in data.read().split("\n")])

with open("values_ADC_for_80mm_kalib.txt", "r") as data:
    values_ADC_for_80mm_kalib = np.array([i for i in data.read().split("\n")])

with open("values_ADC_for_100mm_kalib.txt", "r") as data:
    values_ADC_for_100mm_kalib = np.array([i for i in data.read().split("\n")])

with open("values_ADC_for_120mm_kalib.txt", "r") as data:
    values_ADC_for_120mm_kalib = np.array([i for i in data.read().split("\n")])
...
sigma = (0.25*(sr_znach_ADC/sr_znach_deep - func[0]**2))
print("Погрешность:", sigma)

plt.scatter(average_values_for_ADC, deep)
plt.plot(average_values_for_ADC, func(average_values_for_ADC), label = "h = ", color = "red") #Написать в нахвание коэффициенты
plt.minorticks_on()
